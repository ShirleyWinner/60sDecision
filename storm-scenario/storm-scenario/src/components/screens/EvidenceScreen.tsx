...
export function EvidenceScreen({
  ...
  return (
    ...
    >
      <div className="advisor-banner"><div className={`art art-${agent.id}`} aria-hidden="true" /><strong>{agent.name}</strong><span>FIELD INTELLIGENCE</span></div>
      ...
      <article className="intel-card">
        ...
        <div className={`evidence-visual art ${index===3?'art-vessel':index===4?'art-bridge':'art-storm'}`} aria-hidden="true"><span>EXHIBIT 0{index+1} / ILLUSTRATION</span></div>
        <h2>{item.title}</h2>
