...
export function OutcomeScreen({ outcome, onDebrief, inert }: OutcomeScreenProps) {
  ...
  return (
    ...
    >
      <div className={`outcome-illustration art art-vessel ${outcome === 'balanced'?'safe-voyage':''}`} aria-hidden="true"><span>OPERATION COMPLETE</span></div>
      <div className={`result-medal ${outcome !== 'balanced' ? 'warning' : ''}`}>{copy.medal} {copy.title}</div>
